require(['page', 'entity'], function(Page, entity){
	Page.create('fit/tssd/tssd', function($, $c, tab, me){
		var form = tab.find('form'),
			subForm,
			dialog = tab.find('.dialog'),
			tssdClient = tab.find('div.tssd-client'),
			content = tab.find('div.tssd-client-content'),
		...
		return {
			_init: function(){
				tssdClient.panel({    
					  title: 'TSSD客户端配置<span style="font-size:10px;color:red;">&nbsp;&nbsp;点击右边保存按钮可将配置保存，以便之后复用</span>',  
					  collapsible:true, 
					  collapsed: true,
					  tools: [{    
						title: '保存客户端配置',
				...
				$c.ajaxSubmit({
    				url: $c.getApi('fit.tssds', '/deserializers'),
    				method: 'GET'
    			}, function(data){
				...
				entity.setEntityCombos('TSSD', form.find('[name=tssdClient]'), me.onEntityChange);
				me.onEntityChange();
			},
